...
class LoadTestDataCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $categoryNames = ['OE/PM', 'OE/GM', 'TAMCA/PM', 'TAMCA/GM', 'AP'];
        $categories = [];
        ...
        // 3) Créer les taux de catégories (CategoryTM)
        $categoryTMs = [];
        foreach ($categories as $category) {
            foreach ($grpPerfs as $grpPerf) {
                ...
                $TM->setCategory($category)
                    ->setGrpPerf($grpPerf)
                    ->setTM(rand(10, 50) / 100); // Taux entre 0.10 et 0.50
                $this->em->persist($TM);
                $categoryTMs[] = $TM;
            }
        }
            ...
            foreach ($serviceNames as $srvName) {
                ...
                for ($e = 0; $e < $nbEmployees; $e++) {
                    ...
                    $employee->setMatricule($matricule)
                        ...
                        ->setGrpPerf($randomGrpPerf); // Attribution du groupe de performance
